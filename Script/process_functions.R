  ...
  if(!is.na(speciesdb.name) && speciesdb.name %in% c("FLYBASE","WORMBASE","TAIR")){
    ...
    ncbi.data$ENSEMBL <- sapply(ncbi.data$ENSEMBL,function(x){
      if(is.na(x)) {
        ...
        list.ids <- unlist(strsplit(x,"\\|"))
        filtered.ids <- list.ids[grep("^Ensembl.*:|^FungiDB.*:",list.ids,ignore.case=T)]
        res <- paste0(gsub(".*:","",filtered.ids,ignore.case=T), collapse="|")
        return(ifelse(res=="",NA,res))
      }
    })
  }
...
process.ncbi.archive.data <- function(ncbi.archive.data,global.ncbi.discontinued.data,speciesdb.name)
{
  ...
  if(nrow(global.ncbi.discontinued.data)>0){
...
process.ensembl.data <- function(ensembl.data,speciesdb.name,ncbi.data,hcop.data,species.taxid,is.archive)
{
  #Reconcile gene symbols with NCBI gene symbol data
  colnames(ensembl.data) <- c("ALIAS ENS","GENETYPE","ENSEMBL","GENENAME")
  ensembl.data <- ensembl.data[!is.na(ensembl.data$ENSEMBL),]
  ensembl.data[startsWith(ensembl.data$ENSEMBL,"ENSRNA"),"ENSEMBL"] <- NA
  ensembl.data <- ensembl.data[!is.na(ensembl.data$ENSEMBL),]
  if(39947 %in% species.taxid){
    #Fix Ensembl IDs for Rice
    ensembl.data$ENSEMBL <- ifelse(startsWith(ensembl.data$ENSEMBL,"OS"),
      paste0(substr(ensembl.data$ENSEMBL,1,1),tolower(substr(ensembl.data$ENSEMBL,2,nchar(ensembl.data$ENSEMBL)))),
      ensembl.data$ENSEMBL)
  }
  map.ens.ids.to.symbol <- map.keys.to.values(ncbi.data,ensembl.data$ENSEMBL,"SYMBOL","ENSEMBL NCBI")
  colnames(map.ens.ids.to.symbol) <- c("ENSEMBL","ALIAS NCBI")
  ensembl.data <- merge(ensembl.data,map.ens.ids.to.symbol,all.x=T)
  ensembl.data$SYMBOL <- ifelse(is.na(ensembl.data$`ALIAS NCBI`),ensembl.data$`ALIAS ENS`,ensembl.data$`ALIAS NCBI`)
  ensembl.data$ALIAS <- merge.single.column(ensembl.data,"ALIAS",single.val=F)
  ensembl.data$`ALIAS NCBI` <- NULL
  ensembl.data$`ALIAS ENS` <- NULL
  ...
  if(!is.na(speciesdb.name) && speciesdb.name=="ZFIN"){
    ...
    hcop.data <- unique(hcop.data[hcop.data$ortholog_species==species.taxid,c("ortholog_species_entrez_gene","ortholog_species_ensembl_gene")])
    ...
    ensembl.data$ENTREZID <- merge.single.column(ensembl.data,"ENTREZID",single.val=T)
  ...
  if(nrow(ensembl.data)>0){
    ...
    if(3708 %in% species.taxid){
      #Make gene symbols upper case for Rapeseed 
...
process.uniprot.data <- function(uniprot.data,species)
{
  uniprot.data <- uniprot.data[uniprot.data$V2 %in% c("Gene_Name","GeneID","Ensembl","EnsemblGenome"),c("V1","V2","V3")]
  uniprot.data <- as.data.frame(pivot_wider(uniprot.data, names_from = V2, values_from = V3, 
                  values_fn = ~ paste(.x, collapse = "|")))
  colnames(uniprot.data)[colnames(uniprot.data)=="V1"] <- "UNIPROT"
  colnames(uniprot.data)[colnames(uniprot.data) %in% c("Ensembl","EnsemblGenome")] <- "ENSEMBL"
  colnames(uniprot.data)[colnames(uniprot.data)=="GeneID"] <- "ENTREZID"
  colnames(uniprot.data)[colnames(uniprot.data)=="Gene_Name"] <- "SYMBOL"
  if("ENSEMBL" %in% colnames(uniprot.data)){
    uniprot.data <- uniprot.data %>% separate_rows(all_of(c("ENSEMBL")),sep="\\|")
    uniprot.data$ENSEMBL <- ifelse(is.na(uniprot.data$ENSEMBL),NA,sub("\\.[0-9]+$","",uniprot.data$ENSEMBL))
  } else {
    uniprot.data$ENSEMBL <- NA
  }
  if("ENTREZID" %in% colnames(uniprot.data)){
    uniprot.data <- uniprot.data %>% separate_rows(all_of(c("ENTREZID")),sep="\\|")
  } else {
    uniprot.data$ENTREZID <- NA
  }
  uniprot.data <- as.data.frame(uniprot.data %>% separate_rows(all_of(c("SYMBOL")),sep="\\|"))
  colnames(uniprot.data)[colnames(uniprot.data)=="SYMBOL"] <- "SYMBOL UNIPROT"
  return(uniprot.data)
}
...
process.hgnc.data <- function(speciesdb.data)
{
  ...
  colnames(speciesdb.data) <- c("SYMBOL","GENETYPE SPECIAL","GENENAME SPECIAL","ALIAS SPECIAL","HGNC SPECIAL")
...
process.mgi.data <- function(speciesdb.data)
{
  ...
  colnames(speciesdb.data) <- c("SYMBOL","ALIAS SPECIAL","GENETYPE SPECIAL","GENENAME SPECIAL","MGI SPECIAL")
...
process.xenbase.data <- function(speciesdb.data)
{
  speciesdb.data <- speciesdb.data[,c("V3","V5","V4","V2")]
  colnames(speciesdb.data) <- c("SYMBOL","ALIAS","GENENAME","XENBASE")
  speciesdb.data[speciesdb.data==""] <- NA
  speciesdb.data <- speciesdb.data[!is.na(speciesdb.data$SYMBOL),]
  speciesdb.data$ALIAS <- ifelse(is.na(speciesdb.data$ALIAS),speciesdb.data$SYMBOL,
   paste0(speciesdb.data$ALIAS,"|",speciesdb.data$SYMBOL))
  speciesdb.data <- speciesdb.data %>% group_by(SYMBOL) %>% summarise(across(everything(),function(x){
    unique.info <- unique(unlist(strsplit(as.character(x),"\\|")))
    res <- paste0(unique.info[!is.na(unique.info)],collapse="|")
    ifelse(res=="",NA,res)
  }))
  colnames(speciesdb.data) <- c("SYMBOL","ALIAS SPECIAL","GENENAME SPECIAL","XENBASE SPECIAL")
  return(speciesdb.data)
}

process.vgnc.data <- function(speciesdb.data)
{
  speciesdb.data <- speciesdb.data[,c("symbol","locus_group","name","alias_symbol","vgnc_id")]
  colnames(speciesdb.data) <- c("SYMBOL","GENETYPE SPECIAL","GENENAME SPECIAL","ALIAS SPECIAL","VGNC SPECIAL")
  speciesdb.data$`ALIAS SPECIAL` <- ifelse(is.na(speciesdb.data$`ALIAS SPECIAL`),speciesdb.data$SYMBOL,paste0(speciesdb.data$`ALIAS SPECIAL`,"|",speciesdb.data$SYMBOL))
  speciesdb.data <- speciesdb.data %>% group_by(SYMBOL) %>% summarise(across(everything(),function(x){
    unique.info <- unique(x)
    res <- paste0(unique.info[!is.na(unique.info)],collapse="|")
...
process.ncbi.orthologs.data <- function(global.ncbi.orthologs,species.taxid,taxonomy.table,annotation.data.list)
{
  ...
  ncbi.orthologs.ref <- global.ncbi.orthologs[global.ncbi.orthologs$`#tax_id` %in% species.taxid,]
  ncbi.orthologs.other <- global.ncbi.orthologs[global.ncbi.orthologs$`Other_tax_id` %in% species.taxid,]
...
process.ensembl.orthologs.data <- function(ensembl.orthologs,species.taxid,taxonomy.table,annotation.data.list)
{
  #Rename columns
  ...
  #Map ids to gene symbols
  ensembl.orthologs <- map.ortho.ids.to.symbol(annotation.data.list,ensembl.orthologs,species.taxid,taxonomy.table,"ENSEMBL")
  #Reshape orthology data with one column for each species
  ensembl.orthologs <- reshape.orthology.data(ensembl.orthologs,species.taxid,taxonomy.table)
...
process.alliance.orthologs.data <- function(global.alliance.orthologs,species.taxid,taxonomy.table)
{
  #Filter data about the requested species and the species present in the geneslator DB
  alliance.orthologs <- global.alliance.orthologs
  ...
  alliance.orthologs <- alliance.orthologs[alliance.orthologs$Gene1SpeciesTaxonID %in% species.taxid | alliance.orthologs$Gene2SpeciesTaxonID %in% species.taxid,]
  alliance.orthologs.ref <- alliance.orthologs[alliance.orthologs$Gene1SpeciesTaxonID %in% species.taxid,]
  alliance.orthologs.other <- alliance.orthologs[alliance.orthologs$Gene2SpeciesTaxonID %in% species.taxid,]
  ...
  #Reshape orthology data with one column for each species
  alliance.orthologs <- reshape.orthology.data(alliance.orthologs,species.taxid,taxonomy.table)
...
process.go.data <- function(go.data,go.dictionary,annotation.data.list,species)
{
  ...
  go.data <- go.data[!is.na(go.data$SYMBOL),]
  go.data <- go.data %>% separate_rows(all_of(c("GO","GOEVIDENCE","GONAME","GOTYPE")),sep="\\|")
  go.data$UNIPROT <- NULL
  go.data <- unique(go.data)
  go.data <- go.data %>% group_by(SYMBOL) %>% summarise(across(everything(),function(x){
    unique.info <- unlist(strsplit(as.character(x),"\\|"))
    res <- paste0(unique.info[!is.na(unique.info)],collapse="|")
    ifelse(res=="",NA,res)
  }))
